/**
 * WarrantValidationPanel — shows all errors, warnings, info from validateArticles()
 */
import React from 'react';
import { XCircle, AlertTriangle, Info, CheckCircle } from 'lucide-react';

export default function WarrantValidationPanel({ validation, gaps }) {
  const { errors = [], warnings = [], info = [] } = validation;
  const total = errors.length + warnings.length;

  if (total === 0 && gaps.length === 0) {
    return (
      <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 flex items-center gap-2">
        <CheckCircle className="h-4 w-4 text-emerald-600 flex-shrink-0" />
        <p className="text-xs font-semibold text-emerald-800">All articles validated — no issues found.</p>
      </div>
    );
  }
      ...
      {errors.map((e, i) => (
        <div key={i} className="rounded-xl border border-red-200 bg-red-50 px-3 py-2 flex items-start gap-2">
          ...
          <div>
            ...
            {e.field && <p className="text-[9px] text-red-500 mt-0.5">Field: {e.field}</p>}
          </div>
        </div>
      ))}
      {gaps.length > 0 && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 flex items-start gap-2">
          ...
          <p className="text-[11px] text-amber-800">
            Article numbering gaps detected: missing {gaps.join(', ')}. Verify these are intentional.
          </p>
        </div>
      )}
      {warnings.map((w, i) => (
